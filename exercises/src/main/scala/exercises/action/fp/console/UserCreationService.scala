...
class UserCreationService(console: Console, clock: Clock) {
  ...
  val readDateOfBirth: IO[LocalDate] = {
    val printErrorMessage = writeLine("""Incorrect format, for example enter "18-03-2001" for 18th of March 2001""")
    for {
      _           <- writeLine("What's your date of birth? [dd-mm-yyyy]")
      input       <- readLine
      dateOfBirth <- parseDateOfBirth(input).onError(_ => printErrorMessage)
    } yield dateOfBirth
  }
  ...
  val readSubscribeToMailingList: IO[Boolean] = {
    val printErrorMessage = writeLine("""Incorrect format, enter "Y" for Yes or "N" for "No"""")
    for {
      _         <- writeLine("Would you like to subscribe to our mailing list? [Y/N]")
      input     <- readLine
      subscribe <- parseLineToBoolean(input).onError(_ => printErrorMessage)
    } yield subscribe
  }
